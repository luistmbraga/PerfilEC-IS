...
from Maquina2.Business.Models.Utente import Utente
...
class UtenteDAO:
    ...
    def insertUtenteComId(self, idU, patientName, adminisSex, patientAddress, phNumberHome, phNumberWork, mariStatus, ssnNumber,
                     citizenship, nationality):
        cursor = self.connector.cursor(buffered=True)
        insert = ("INSERT INTO Utente "
                  "(idUtente, morada, telefoneCasa, telefoneTrabalho, nome, sexo, estadoCivil, ssn, nacionalidade, cidadania)"
                  "VALUES (%(id)s, %(morada)s, %(telefoneCasa)s, %(telefoneTrabalho)s, %(nome)s, %(sexo)s, %(estadoCivil)s, %(ssn)s, %(nacionalidade)s, %(cidadania)s )")
        dados = {
            'id': idU,
            'morada': patientAddress,
            'telefoneCasa': phNumberHome,
